...
const questions = [
  ...
  },
  {
    ...
    questionTextKr: '당신이 시험 중 부정행위를 목격한다면, 어떻게 대처할 것인가요?',
    options: [
    ...
    <div className="poll min-h-screen flex flex-col items-center justify-center">
      ...
      {finalResult === null ? (
        showConfused ? (
          <div>
            <Confused />
            <Button onClick={handleGiveHint}>{"Give a hint to the Sorting Hat >>"}</Button>
          </div>
        ) : (
          <Question
            questionText={showBonusQuestion ? bonusQuestion.questionText : questions[currentQuestionIndex].questionText}
            questionTextKr={showBonusQuestion ? bonusQuestion.questionTextKr : questions[currentQuestionIndex].questionTextKr}
            options={showBonusQuestion ? bonusQuestion.options : questions[currentQuestionIndex].options}
            optionsKr={showBonusQuestion ? bonusQuestion.optionsKr : questions[currentQuestionIndex].optionsKr}
